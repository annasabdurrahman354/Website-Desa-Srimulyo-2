<?php

namespace App\Http\Controllers\Api\V1\Admin;

use App\Http\Controllers\Controller;
use App\Http\Requests\StoreKategoriArtikelRequest;
use App\Http\Requests\UpdateKategoriArtikelRequest;
use App\Http\Resources\Admin\KategoriArtikelResource;
use App\Models\KategoriArtikel;
use Gate;
use Illuminate\Http\Request;
use Illuminate\Http\Response;

class KategoriArtikelApiController extends Controller
{
    public function index()
    {
        abort_if(Gate::denies('kategori_artikel_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        return new KategoriArtikelResource(KategoriArtikel::all());
    }

    public function store(StoreKategoriArtikelRequest $request)
    {
        $kategoriArtikel = KategoriArtikel::create($request->validated());

        return (new KategoriArtikelResource($kategoriArtikel))
            ->response()
            ->setStatusCode(Response::HTTP_CREATED);
    }

    public function show(KategoriArtikel $kategoriArtikel)
    {
        abort_if(Gate::denies('kategori_artikel_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        return new KategoriArtikelResource($kategoriArtikel);
    }

    public function update(UpdateKategoriArtikelRequest $request, KategoriArtikel $kategoriArtikel)
    {
        $kategoriArtikel->update($request->validated());

        return (new KategoriArtikelResource($kategoriArtikel))
            ->response()
            ->setStatusCode(Response::HTTP_ACCEPTED);
    }

    public function destroy(KategoriArtikel $kategoriArtikel)
    {
        abort_if(Gate::denies('kategori_artikel_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $kategoriArtikel->delete();

        return response(null, Response::HTTP_NO_CONTENT);
    }
}
