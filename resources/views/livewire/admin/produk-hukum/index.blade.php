    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="ProdukHukum" format="csv" />
                <livewire:excel-export model="ProdukHukum" format="xlsx" />
                <livewire:excel-export model="ProdukHukum" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        ...
                        <th>
                            ...
                            @include('components.table.sort', ['field' => 'tahun'])
                        </th>
                        <th>
                ...
                <tbody>
                    @forelse($produkHukums as $produkHukum)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $produkHukum->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $produkHukum->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('produk_hukum_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.produk-hukums.show', $produkHukum) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('produk_hukum_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.produk-hukums.edit', $produkHukum) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('produk_hukum_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $produkHukum->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
