         ...
         <div class="border bg-white border-gray-300 rounded-md shadow-sm lg:col-span-4 lg:my-4 w-full h-fit">
            <div class="flex flex-col"> 
               ...
               <div class="w-full h-fit px-4 py-4">
                     ...
                     @else
                        <iframe
                           src="https://drive.google.com/viewerng/viewer?embedded=true&url={{$dokumenHukum->getFirstMediaUrl('dokumen_umum_berkas_dokumen')}}"
                           frameBorder="0"
                           scrolling="auto"
                           height="100vh"
                           width="100%"
...
@push('scripts')
      <script>
         document.getElementById('button-share').addEventListener('click', event => {
            if (navigator.share) {
               navigator.share({
                  title: '{!!$dokumenUmum->judul!!}',
                  url: window.location.href,
               }).then(() => {
                  console.log('Thanks for sharing! {!!$dokumenUmum->judul!!} in '+ window.location.href);
               }).catch(console.error);
            }
         });
...
@push('scripts')
      ...
      <script>
         ...
         var myPDF = PDFObject.embed("{{$dokumenUmum->getFirstMediaUrl('dokumen_umum_berkas_dokumen')}}", "#pdf", options);
      </script>
      <script>
         document.getElementById('button-share').addEventListener('click', event => {
            if (navigator.share) {
