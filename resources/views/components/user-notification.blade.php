  ...
  <div class="divide-y divide-gray-100 dark:divide-gray-700">
    @forelse(auth()->user()->alerts()->latest()->take(10)->get() as $alert)
      @if ($alert->pivot->seen_at == null)
        <a href="{{$alert->link ?? "#"}}" class="flex px-4 py-3 bg-green-100 hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      @else
        <a href="{{$alert->link ?? "#"}}" class="flex px-4 py-3 bg-white hover:bg-gray-100 dark:hover:bg-gray-700">
          <div class="w-full pl-3">
              <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">{{$alert->message}}</div>
              <div class="text-xs text-blue-600 dark:text-blue-500">{{$alert->created_at->diffForHumans()}}</div>
          </div>
    ...
    @empty
      <div class="flex p-4 text-base text-blue-800 border border-blue-300 bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800" role="alert">
        ...
        <div>
          <span class="font-medium">Info!</span> Belum ada notifikasi untuk saat ini.
        </div>
      </div>
    @endforelse
  </div>
  <a href="#" class="block py-2 text-base font-medium text-center text-gray-900 rounded-b-lg bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white">
